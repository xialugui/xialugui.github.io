...
#import "primitives.typ": event, eventspan
#import "functions.typ": eventspanlocation, eventlocation
...
#let timeline(
  ...
  cetz.canvas(
    // the canvas draw surface
    {
      ...
      line(timeline-startpoint, timeline-endpoint, name: "Timespan", mark: (end: ">"), stroke: linestroke)
      ...
      content((timeline-endpoint.first() + endyear-distance-timeline, 0), [ #endyear#[s] ])
      ...
      while interval-year < endyear {
        let posx = timeline-endpoint.first() * eventspanlocation(start-year: startyear, end-year: endyear, eventyear: interval-year)

        // the line marking itself
        line((posx, -0.1), (posx, 0.1), stroke: linestroke)

        // year description marking
        if interval-year != startyear {
          if startyear <= 0 {
            content((posx, -0.3), [#interval-year])
          }
      ...
      if events.len() != 0 {
        for x in events {
          let event-pos = (timeline-endpoint.first() * eventlocation(start-year: startyear, end-year: endyear, event: x), 0)

          // line for vertical marking
          let line-pos1 = (event-pos.first(), 0.4)
          line(event-pos, line-pos1, stroke: linestroke)

          // content descriptions for the year
          content((event-pos.first(), 0.7), [ #x.year ])
          // content descriptions for event

          content((event-pos.first(), line-pos1.last() + 0.7), angle: 45deg, anchor: "base-west", [ #x.title ])
        }
      }
      ...
      if eventspans.len() != 0 {
        for x in eventspans {
          let event-pos-x-0 = timeline-endpoint.first() * eventspanlocation(start-year: startyear, end-year: endyear, eventyear: x.start-point)
          let event-pos-x-1 = timeline-endpoint.first() * eventspanlocation(start-year: startyear, end-year: endyear, eventyear: x.end-point)
          ...
          if spanheight-negative-y == 0 and spanheight-positive-y == 0 {
            ...
            rect(
              (event-pos-x-0, span-y-offset),
              (event-pos-x-1, -span-y-offset),
          ...
          if spanheight-negative-y != 0 or spanheight-positive-y != 0 {
            // block of the event span
            rect(
              (event-pos-x-0, spanheight-positive-y),
              (event-pos-x-1, -spanheight-negative-y),
              fill: x.color.transparentize(60%),
              stroke: x.color.transparentize(60%),
              radius: 5pt,
            )
          ...
          content(
            ...
            anchor: "mid",
            box(width: 4em)[ #align(center, x.title) ],
          )
        }
      }
    },
